...
gauss_auSable_randMiss <- readRDS(here("data/missingDatasets/gauss_real_auSable_randMiss_full.rds"))
...
fit_arima_MI <- function(sim_list, sim_pars, imputationsnum, forecast = TRUE, forecast_days = 365,
  ...
  if(forecast){  
    ...
    return(list(arima_pars = paramlistsim, 
                arima_se = selistsim
    ))
  }
...
if (!dir.exists("data/model_results/gauss_real_MAR_arima_modResults/au_sable_paramRecovery")) {
  dir.create("data/model_results/gauss_real_MAR_arima_modResults/au_sable_paramRecovery")
}
...
dirname <- "./data/model_results/gauss_real_MAR_arima_modResults/au_sable_paramRecovery/"
...
# run models with the complete dataset (no missingness) to have a reference point ----------

  OutFile_params <- paste(dirname, "CompleteDataset_arimavals.csv", sep = "")
 
  # make sure sim_list and sim_params are not pointing to the whole list, which starts w character elements
  sim_list<- list("y" = c(gauss_auSable_randMiss[[1]]$y$y))
  ...
  # save arima model parameters
  arimadrop_MAR_df <- arima_drop_MAR$arima_pars
  arimadrop_MAR_df$missingness <- NA
  arimadrop_MAR_df$type <- "dropNA_simple"
  arimadrop_MAR_df$run_no <- NA
  arimadrop_MAR_df$missingprop_autocor <- "full_dataset"
  ...
  # save arima model parameters
  arimadropCC_MAR_df <- arima_dropCC_MAR$arima_pars
  arimadropCC_MAR_df$missingness <- NA
  arimadropCC_MAR_df$type <- 'dropNA_complete'
  arimadropCC_MAR_df$run_no <- NA
  arimadropCC_MAR_df$missingprop_autocor <- "full_dataset"
  ...
  # save arima model parameters
  arimaKalman_MAR_df <- arima_kalman_MAR$arima_pars
  arimaKalman_MAR_df$missingness <- NA
  arimaKalman_MAR_df$type <- 'Kalman Filter'
  arimaKalman_MAR_df$run_no <- NA
  arimaKalman_MAR_df$missingprop_autocor <- "full_dataset"
  ...
  # save arima model parameters
  arimaMI_MAR_df <- arima_mi_MAR$arima_pars
  arimaMI_MAR_df$missingness <- NA
  arimaMI_MAR_df$type <- 'Multiple Imputations'
  arimaMI_MAR_df$run_no <- NA
  arimaMI_MAR_df$missingprop_autocor <- "full_dataset"
