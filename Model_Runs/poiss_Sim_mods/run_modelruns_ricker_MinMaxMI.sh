...
Rscript Model_Runs/poiss_Sim_mods/modelruns_ricker_MI.R ${datFile} ${parFile} ${clsize} ${saveFile} ${index1} ${index2} ${seed} > Model_Runs/poiss_trial/outputRickerMinMaxMI_${SLURM_ARRAY_TASK_ID}.txt
...
then
    ...
    Rscript Model_Runs/poiss_Sim_mods/modelruns_ricker_MI.R ${datFile} ${parFile} ${clsize} ${saveFile} ${index1} ${index2} $newseed $cur_runs > Model_Runs/poiss_trial/outputRickerMinMaxMI_${SLURM_ARRAY_TASK_ID}.txt
    ((cur_runs++))
fi
