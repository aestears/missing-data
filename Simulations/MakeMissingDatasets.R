...
for (i in 1:length(inputAutocor)) {
  ...
  if (i == 1) {
    assign(x = paste0("gauss_sim_randMiss_full_autoCorr_0") , 
           value = tempOutList)
  } else {
    assign(x = paste0("gauss_sim_randMiss_full_autoCorr_", 
...
gauss_sim_minMaxMiss_full <- lapply(X = gauss_sim, FUN = function(x) 
  list("y" = makeMissing(timeSeries = x$y, typeMissing = "minMax", type =  "Gaussian"), 
       
       "sim_params" <- x$sim_params)
)

for (i in 1:length(gauss_sim_minMaxMiss_full)) {
  names(gauss_sim_minMaxMiss_full[[i]]) <- c("y", "sim_params")
  gauss_sim_minMaxMiss_full[[i]]$y <- c(list("y_noMiss" = gauss_sim[[i]]$y), gauss_sim_minMaxMiss_full[[i]]$y)
...
for (i in 1:(reps*length(inputAutocor))) {
  ...
  #   assign(x = paste0("gauss_auSable_randMiss_full_autoCorr_0_i",rep(1:50, each = 4)[i]) , 
  #          value = tempOutDf)
  # } else 
  {
    assign(x = paste0("gauss_auSable_randMiss_full_autoCorr_", 
           ...
           value = tempOutDf)
    listNames[i] <- paste0("gauss_auSable_randMiss_full_autoCorr_", 
                           str_pad(str_extract_all(string = inputAutocor[((i-1)%%length(inputAutocor))+1], pattern = "\\d+" , simplify = TRUE)[,2], width = 2, side = "right", pad = "0"),"_i",rep(1:50, each = 4)[i])
  }
}
...
gauss_auSable_randMiss_full_list <- vector(mode = "list", length = 150) 
names(gauss_auSable_randMiss_full_list) <- listNames
for (i in 1:length(listNames)) {
  # get the correct autocorrelation/missing data data.frame
  tempDF <- get(x = listNames[i])
  # change into a list element
  gauss_auSable_randMiss_full_list[[i]]$y <- tempDF %>% 
    rename_with(~ str_replace(string = names(tempDF), pattern = "GPP_", replacement = "")) %>% 
    rename("y" = "GPP" ) %>% 
    dplyr::select(-date, -ER, -light, -Q) %>% 
    as.list()
  gauss_auSable_randMiss_full_list[[i]]$sim_params$date <- tempDF %>% 
    dplyr::select(date)
  gauss_auSable_randMiss_full_list[[i]]$sim_params$light <- tempDF %>% 
    dplyr::select(light)
  gauss_auSable_randMiss_full_list[[i]]$sim_params$Q <- tempDF %>% 
    dplyr::select(Q)
}

gauss_auSable_randMiss_full <- gauss_auSable_randMiss_full_list
...
gauss_auSable_minMaxMiss_full_TEMP <- as.data.frame(makeMissing(timeSeries = gauss_auSable$GPP, 
                                                           typeMissing = "minMax", 
                                                           propMiss = inputPropMiss, type =  "Gaussian"))


#names(gauss_auSable_minMaxMiss_full_TEMP) <- paste0("GPP_",names(gauss_auSable_minMaxMiss_full_TEMP))

gauss_auSable_minMaxMiss_full <- cbind(gauss_auSable, gauss_auSable_minMaxMiss_full_TEMP)

# change "broods" to "y" in the data.frame
names(gauss_auSable_minMaxMiss_full)[2] <- "y" 

# transform into a list (for consistency w/ simulated data)
gauss_auSable_minMaxMiss_full_list <- vector(mode = "list", length = 1) 
gauss_auSable_minMaxMiss_full_list[[1]]$y <- as.list(gauss_auSable_minMaxMiss_full[c(2,9:ncol(gauss_auSable_minMaxMiss_full))])
gauss_auSable_minMaxMiss_full_list[[1]]$sim_params$date <- gauss_auSable_minMaxMiss_full$date 
gauss_auSable_minMaxMiss_full_list[[1]]$sim_params$light <- gauss_auSable_minMaxMiss_full$light
gauss_auSable_minMaxMiss_full_list[[1]]$sim_params$light.rel <- gauss_auSable_minMaxMiss_full$light.rel
gauss_auSable_minMaxMiss_full_list[[1]]$sim_params$Q <- gauss_auSable_minMaxMiss_full$Q

gauss_auSable_minMaxMiss_full <- gauss_auSable_minMaxMiss_full_list
...
# cut off the last 10 values before going to missingness procedure so we don't need to recalculate
#gauss_badger <- gauss_badger[1:729,]
...
for (i in 1:(reps*length(inputAutocor))) {
  ...
  while(!check){
    propMiss=c(0.2,0.4,0.6)
    tempOutDf <- as.data.frame(makeMissing(timeSeries = gauss_badger$GPP, propMiss = propMiss,
                                           typeMissing = "random", 
                                           autoCorr = inputAutocor[((i-1)%%length(inputAutocor))+1]))
  ...
  # rename the output list to reflect the input autocorrelation
  # if (((i-1)%%length(inputAutocor))+1 == 1) {
  #   assign(x = paste0("gauss_badger_randMiss_full_autoCorr_0_i",rep(1:50, each = 4)[i]) , 
  #          value = tempOutDf)
  # } else 
  {
    assign(x = paste0("gauss_badger_randMiss_full_autoCorr_", 
                      str_pad(str_extract_all(string = inputAutocor[((i-1)%%length(inputAutocor))+1], pattern = "\\d+" , simplify = TRUE)[,2], width = 2, side = "right", pad = "0"),"_i",rep(1:50, each = 4)[i]) , 
           value = tempOutDf)
    listNames[i] <- paste0("gauss_badger_randMiss_full_autoCorr_", 
                           str_pad(str_extract_all(string = inputAutocor[((i-1)%%length(inputAutocor))+1], pattern = "\\d+" , simplify = TRUE)[,2], width = 2, side = "right", pad = "0"),"_i",rep(1:50, each = 4)[i])
  }
}
...
# put all of the data into one list
gauss_badger_randMiss_full_list <- vector(mode = "list", length = 150) 
names(gauss_badger_randMiss_full_list) <- listNames
for (i in 1:length(listNames)) {
  # get the correct autocorrelation/missing data data.frame
  tempDF <- get(x = listNames[i])
  # change into a list element
  gauss_badger_randMiss_full_list[[i]]$y <- tempDF %>% 
    rename_with(~ str_replace(string = names(tempDF), pattern = "GPP_", replacement = "")) %>% 
    rename("y" = "GPP" ) %>% 
    dplyr::select(-date, -ER, -light, -Q) %>% 
    as.list()
  gauss_badger_randMiss_full_list[[i]]$sim_params$date <- tempDF %>% 
    dplyr::select(date)
  gauss_badger_randMiss_full_list[[i]]$sim_params$light <- tempDF %>% 
    dplyr::select(light)
  gauss_badger_randMiss_full_list[[i]]$sim_params$Q <- tempDF %>% 
    dplyr::select(Q)
}

gauss_badger_randMiss_full <- gauss_badger_randMiss_full_list

## missing in min and max of data
## for the MNAR data, we need to do slightly different propMissing values, since the same proportion will cutoff the same exact values 

inputPropMiss <- c(.18, .19, .2, .21, .22, .38, .39, .4, .41, .42, .58, .59, .6, .61, .62)
gauss_badger_minMaxMiss_full_TEMP <- as.data.frame(makeMissing(timeSeries = gauss_badger$GPP, 
                                                           typeMissing = "minMax", 
                                                           propMiss = inputPropMiss, type =  "Gaussian"))
#names(gauss_badger_minMaxMiss_full_TEMP) <- paste0("GPP_",names(gauss_badger_minMaxMiss_full_TEMP))

gauss_badger_minMaxMiss_full <- cbind(gauss_badger, gauss_badger_minMaxMiss_full_TEMP)

# change "broods" to "y" in the data.frame
names(gauss_badger_minMaxMiss_full)[2] <- "y" 

# transform into a list (for consistency w/ simulated data)
gauss_badger_minMaxMiss_full_list <- vector(mode = "list", length = 1) 
gauss_badger_minMaxMiss_full_list[[1]]$y <- as.list(gauss_badger_minMaxMiss_full[c(2,9:ncol(gauss_badger_minMaxMiss_full))])
gauss_badger_minMaxMiss_full_list[[1]]$sim_params$date <- gauss_badger_minMaxMiss_full$date 
gauss_badger_minMaxMiss_full_list[[1]]$sim_params$light <- gauss_badger_minMaxMiss_full$light
gauss_badger_minMaxMiss_full_list[[1]]$sim_params$light.rel <- gauss_badger_minMaxMiss_full$light.rel
gauss_badger_minMaxMiss_full_list[[1]]$sim_params$Q <- gauss_badger_minMaxMiss_full$Q

gauss_badger_minMaxMiss_full <- gauss_badger_minMaxMiss_full_list


# Gaussian Simulated Data  - curtailed for forecasting -------------------------------------------------
# read in data
# remove 20% of observations from the end
...
# Gaussian Real Data (Badger Mill Creek)  - curtailed for forecasting--------------------------------
# read in data
gauss_badger <- read.csv("./data/badger_mill_creek_prepped.csv")
...
for (i in 1:(reps*length(inputAutocor))) {
  ...
  # rename the output list to reflect the input autocorrelation
  # if (((i-1)%%length(inputAutocor))+1 == 1) {
  #   assign(x = paste0("gauss_badger_randMiss_trim_autoCorr_0_i",rep(1:50, each = 4)[i]) , 
  #          value = tempOutDf)
  # } else 
  {
    assign(x = paste0("gauss_badger_randMiss_trim_autoCorr_", 
                      str_pad(str_extract_all(string = inputAutocor[((i-1)%%length(inputAutocor))+1], pattern = "\\d+" , simplify = TRUE)[,2], width = 2, side = "right", pad = "0"),"_i",rep(1:50, each = 4)[i]) , 
           value = tempOutDf)
    listNames[i] <- paste0("gauss_badger_randMiss_trim_autoCorr_", 
                           str_pad(str_extract_all(string = inputAutocor[((i-1)%%length(inputAutocor))+1], pattern = "\\d+" , simplify = TRUE)[,2], width = 2, side = "right", pad = "0"),"_i",rep(1:50, each = 4)[i])
  }
}
...
# put all of the data into one list
gauss_badger_randMiss_trim_list <- vector(mode = "list", length = 150) 
names(gauss_badger_randMiss_trim_list) <- listNames
for (i in 1:length(listNames)) {
  # get the correct autocorrelation/missing data data.frame
  tempDF <- get(x = listNames[i])
  # change into a list element
  gauss_badger_randMiss_trim_list[[i]]$y <- tempDF %>% 
    rename_with(~ str_replace(string = names(tempDF), pattern = "GPP_", replacement = "")) %>% 
    rename("y" = "GPP" ) %>% 
    dplyr::select(-date, -ER, -light, -Q) %>% 
    as.list()
  gauss_badger_randMiss_trim_list[[i]]$sim_params$date <- tempDF %>% 
    dplyr::select(date)
  gauss_badger_randMiss_trim_list[[i]]$sim_params$light <- tempDF %>% 
    dplyr::select(light)
  gauss_badger_randMiss_trim_list[[i]]$sim_params$Q <- tempDF %>% 
    dplyr::select(Q)
}

gauss_badger_randMiss_trim <- gauss_badger_randMiss_trim_list

## missing in min and max of data
## for the MNAR data, we need to do slightly different propMissing values, since the same proportion will cutoff the same exact values 

inputPropMiss <- c(.18, .19, .2, .21, .22, .38, .39, .4, .41, .42, .58, .59, .6, .61, .62)
gauss_badger_minMaxMiss_trim_TEMP <- as.data.frame(makeMissing(timeSeries = gauss_badger$GPP, 
                                                               typeMissing = "minMax", 
                                                               propMiss = inputPropMiss, type =  "Gaussian"))
#names(gauss_badger_minMaxMiss_trim_TEMP) <- paste0("GPP_",names(gauss_badger_minMaxMiss_trim_TEMP))

gauss_badger_minMaxMiss_trim <- cbind(gauss_badger, gauss_badger_minMaxMiss_trim_TEMP)

# change "broods" to "y" in the data.frame
names(gauss_badger_minMaxMiss_trim)[2] <- "y" 

# transform into a list (for consistency w/ simulated data)
gauss_badger_minMaxMiss_trim_list <- vector(mode = "list", length = 1) 
gauss_badger_minMaxMiss_trim_list[[1]]$y <- as.list(gauss_badger_minMaxMiss_trim[c(2,9:ncol(gauss_badger_minMaxMiss_trim))])
gauss_badger_minMaxMiss_trim_list[[1]]$sim_params$date <- gauss_badger_minMaxMiss_trim$date 
gauss_badger_minMaxMiss_trim_list[[1]]$sim_params$light <- gauss_badger_minMaxMiss_trim$light
gauss_badger_minMaxMiss_trim_list[[1]]$sim_params$light.rel <- gauss_badger_minMaxMiss_trim$light.rel
gauss_badger_minMaxMiss_trim_list[[1]]$sim_params$Q <- gauss_badger_minMaxMiss_trim$Q

gauss_badger_minMaxMiss_trim <- gauss_badger_minMaxMiss_trim_list

# Poisson Simulated Data - full dataset --------------------------------------------------
# read in data
...
for (i in 1:length(inputAutocor)) {
  ...
  # rename the output list to reflect the input autocorrelation
  if (i == 1) {
    assign(x = paste0("pois_sim_randMiss_autoCorr_0_full_") , 
           value = tempOutList)
  } else {
    assign(x = paste0("pois_sim_randMiss_autoCorr_full_", 
                      str_pad(str_extract_all(string = inputAutocor[i], pattern = "\\d+" , simplify = TRUE)[,2], width = 2, side = "right", pad = "0")) , 
           value = tempOutList)
  }
}
...
# missing in min and max of data
pois_sim_minMaxMiss_full <- lapply(X = pois_sim, FUN = function(x) {
  list("y" = makeMissing(timeSeries = x$y, typeMissing = "minMax", type = "Poisson"), 
       "sim_params" <- x$sim_params)
}
)

for (i in 1:length(pois_sim_minMaxMiss_full)) {
  names(pois_sim_minMaxMiss[[i]]) <- c("y", "sim_params")
  pois_sim_minMaxMiss_full[[i]]$y <- c(list("y_noMiss" = pois_sim[[i]]$y), pois_sim_minMaxMiss_full[[i]]$y)
}
...
# Poisson Real Data (Protist data) -  curtailed for forecasting -------------------------------
# remove the last four observations (the last ~19% of observations)
bursaria_trim <- bursaria_diff %>% 
  filter(days < 42)
...
inputAutocor <- c(.25, .50, .75)
reps <- 50
tolerance <- .2#0.05 # have to increase even more b/c the timeseries is so short
patchNames <- unique(bursaria_trim$patch)
for (i in 1:(reps*length(inputAutocor))) {
  # calculate missing vectors with increasing levels of missingness
  ...
  while(!check){
    ...
    tempOutList <- lapply(X = 1:10, FUN = function(x) {
      temp <- cbind(bursaria_trim[bursaria_trim$patch == patchNames[x],c("patch", "date", "number", "days")],
                    
                    as.data.frame(makeMissing(timeSeries = bursaria_trim[bursaria_trim$patch == patchNames[x],"number"], 
                                              propMiss = propMiss,
                                              typeMissing = "random", 
                                              autoCorr = inputAutocor[((i-1)%%length(inputAutocor))+1])))
      # save autocorrelation values
...
for (i in 1:(reps*length(inputAutocor))) {
  # get the correct autocorrelation/missing data data.frame
  tempDF <- get(x = paste0("pois_real_randMiss_autoCorr_", autcorVector[((i-1)%%length(inputAutocor))+1],"_i",rep(1:50, each = 4)[i]))
  # if there are replicate names, add an additional number to differentiate them 
  if(sum(duplicated(names(tempDF))) > 0) {
    names(tempDF)[which(duplicated(names(tempDF)))] <- paste0(names(tempDF[duplicated(names(tempDF))]), ".1")}
  # change into a list element
  pois_real_randMiss_list[[i]]$y <- tempDF %>%
    #rename_with(~ str_replace(string = names(tempDF), pattern = "Broods_", replacement = "")) %>%
    #rename("y" = "count" ) %>%
...
pois_real_randMiss_trim <- pois_real_randMiss_list
...
propMiss=c(.18, .19, .2, .21, .22, .38, .39, .4, .41, .42, .58, .59, .6, .61, .62)
# for each replicate, remove propMiss percent of the observations
tempOutList <- cbind(bursaria_trim[,c("patch", "date", "number", "days")],
                     as.data.frame(makeMissing(timeSeries = bursaria_trim[,"number"], 
                                               propMiss = propMiss,
                                               typeMissing = "minMax", type = "Poisson")))
names(tempOutList)[5:19] <- c("propMiss_0.18", "propMiss_0.19", "propMiss_0.2", "propMiss_0.21", "propMiss_0.22", "propMiss_0.38", 
                              "propMiss_0.39", "propMiss_0.4", "propMiss_0.41", "propMiss_0.42", "propMiss_0.58", 
...
# put all of the data into an organized list
pois_real_minMaxMiss_trim <- list(
  "y" = tempOutList %>% 
    dplyr::select(patch, number, days, 5:19) %>% 
    as.list(), 
  "sim_params" = NA
...
## do this for the gaussiam sim data (full dataset)
...
names(gauss_sim_randMiss_full_autoCorr_0) <- paste0("gauss_sim",1:1000, "_randMiss_autoCorr_0")
names(gauss_sim_randMiss_full_autoCorr_10) <- paste0("gauss_sim",1:1000, "_randMiss_autoCorr_10")
names(gauss_sim_randMiss_full_autoCorr_20) <- paste0("gauss_sim",1:1000, "_randMiss_autoCorr_20")
names(gauss_sim_randMiss_full_autoCorr_30) <- paste0("gauss_sim",1:1000, "_randMiss_autoCorr_30")
names(gauss_sim_randMiss_full_autoCorr_40) <- paste0("gauss_sim",1:1000, "_randMiss_autoCorr_40")
names(gauss_sim_randMiss_full_autoCorr_50) <- paste0("gauss_sim",1:1000, "_randMiss_autoCorr_50")
names(gauss_sim_randMiss_full_autoCorr_60) <- paste0("gauss_sim",1:1000, "_randMiss_autoCorr_60")
names(gauss_sim_randMiss_full_autoCorr_70) <- paste0("gauss_sim",1:1000, "_randMiss_autoCorr_70")
names(gauss_sim_randMiss_full_autoCorr_80) <- paste0("gauss_sim",1:1000, "_randMiss_autoCorr_80")
names(gauss_sim_randMiss_full_autoCorr_90) <- paste0("gauss_sim",1:1000, "_randMiss_autoCorr_90")

gauss_sim_randMiss_full_A <- c(gauss_sim_randMiss_full_autoCorr_0, 
                          gauss_sim_randMiss_full_autoCorr_10,
                          gauss_sim_randMiss_full_autoCorr_20,
                          gauss_sim_randMiss_full_autoCorr_30,
                          gauss_sim_randMiss_full_autoCorr_40
)
gauss_sim_randMiss_full_B <- c(gauss_sim_randMiss_full_autoCorr_50,
                          gauss_sim_randMiss_full_autoCorr_60,
                          gauss_sim_randMiss_full_autoCorr_70,
                          gauss_sim_randMiss_full_autoCorr_80,
                          gauss_sim_randMiss_full_autoCorr_90)

# Remove the y_no miss from all nested lists (Amelia doesn't like NO missing values) 
# for first chunk of data
#test <- gauss_sim_randMiss_full_A
test <- vector(mode = "list", length = 5000)
for (i in 1:length(gauss_sim_randMiss_full_A)) {
  # remove the "y_noMiss" sub_list
  test[[i]]$y <- gauss_sim_randMiss_full_A[[i]]$y[2:16]
  test[[i]]$sim_params$X <- gauss_sim_randMiss_full_A[[i]]$sim$X
}
# rename w/ accurate names
names(test) <- names(gauss_sim_randMiss_full_A)
gauss_sim_randMiss_full_A <- test

# for second chunk of data
test <- gauss_sim_randMiss_full_B

for (i in 1:length(gauss_sim_randMiss_full_B)) {
  # remove the "y_noMiss" sub_list
  test[[i]]$y <- gauss_sim_randMiss_full_B[[i]]$y[2:16]
  test[[i]]$sim_params$X <- gauss_sim_randMiss_full_B[[i]]$sim$X
}
# rename w/ accurate names
names(test) <- names(gauss_sim_randMiss_full_B)
gauss_sim_randMiss_full_B <- test
...
names(pois_sim_randMiss_autoCorr_0_full) <- paste0("pois_sim",1:1000, "_randMiss_autoCorr_0")
names(pois_sim_randMiss_autoCorr_full_10) <- paste0("pois_sim",1:1000, "_randMiss_autoCorr_10")
names(pois_sim_randMiss_autoCorr_full_20) <- paste0("pois_sim",1:1000, "_randMiss_autoCorr_20")
names(pois_sim_randMiss_autoCorr_full_30) <- paste0("pois_sim",1:1000, "_randMiss_autoCorr_30")
names(pois_sim_randMiss_autoCorr_full_40) <- paste0("pois_sim",1:1000, "_randMiss_autoCorr_40")
names(pois_sim_randMiss_autoCorr_full_50) <- paste0("pois_sim",1:1000, "_randMiss_autoCorr_50")
names(pois_sim_randMiss_autoCorr_full_60) <- paste0("pois_sim",1:1000, "_randMiss_autoCorr_60")
names(pois_sim_randMiss_autoCorr_full_70) <- paste0("pois_sim",1:1000, "_randMiss_autoCorr_70")
names(pois_sim_randMiss_autoCorr_full_80) <- paste0("pois_sim",1:1000, "_randMiss_autoCorr_80")
names(pois_sim_randMiss_autoCorr_full_90) <- paste0("pois_sim",1:1000, "_randMiss_autoCorr_90")

pois_sim_randMiss_full_A <- c(pois_sim_randMiss_autoCorr_0_full, 
                         pois_sim_randMiss_autoCorr_full_10,
                         pois_sim_randMiss_autoCorr_full_20,
                         pois_sim_randMiss_autoCorr_full_30,
                         pois_sim_randMiss_autoCorr_full_40
)
pois_sim_randMiss_full_B <- c(pois_sim_randMiss_autoCorr_full_50,
                         pois_sim_randMiss_autoCorr_full_60,
                         pois_sim_randMiss_autoCorr_full_70,
                         pois_sim_randMiss_autoCorr_full_80,
                         pois_sim_randMiss_autoCorr_full_90)

# Remove the y_no miss from all nested lists (Amelia doesn't like NO missing values) 
# for first chunk of data
#test <- pois_sim_randMiss_A
test <- vector(mode = "list", length = 5000)
for (i in 1:length(pois_sim_randMiss_full_A)) {
  # remove the "y_noMiss" sub_list
  test[[i]]$y <- pois_sim_randMiss_full_A[[i]]$y[2:16]
  test[[i]]$sim_params$X <- pois_sim_randMiss_full_A[[i]]$sim$X
}
# rename w/ accurate names
names(test) <- names(pois_sim_randMiss_full_A)
pois_sim_randMiss_full_A <- test

# for second chunk of data
test <- pois_sim_randMiss_full_B

for (i in 1:length(pois_sim_randMiss_full_B)) {
  # remove the "y_noMiss" sub_list
  test[[i]]$y <- pois_sim_randMiss_full_B[[i]]$y[2:16]
  test[[i]]$sim_params$X <- pois_sim_randMiss_full_B[[i]]$sim$X
}
# rename w/ accurate names
names(test) <- names(pois_sim_randMiss_full_B)
pois_sim_randMiss_full_B <- test
...
saveRDS(pois_sim_randMiss_full_A, file = "./data/missingDatasets/pois_sim_randMiss_full_A.rds")
saveRDS(pois_sim_randMiss_full_B, file = "./data/missingDatasets/pois_sim_randMiss_full_B.rds")

saveRDS(pois_sim_randMiss_trim_A, file = "./data/missingDatasets/pois_sim_randMiss_trim_A.rds")
saveRDS(pois_sim_randMiss_trim_B, file = "./data/missingDatasets/pois_sim_randMiss_trim_B.rds")
...
saveRDS(pois_sim_minMaxMiss_full, file = "./data/missingDatasets/pois_sim_MinMaxMiss_full.rds")
## save poisson simulated MNAR data
saveRDS(pois_sim_minMaxMiss_20PercTrimmed, file = "./data/missingDatasets/pois_sim_MinMaxMiss_trim.rds")

## save poisson real MNAR data
saveRDS(pois_real_minMaxMiss_full, file = "./data/missingDatasets/pois_real_MinMaxMiss_full.rds")
## save poisson real MAR data
saveRDS(pois_real_randMiss_full, file = "./data/missingDatasets/pois_real_randMiss_full.rds")

## save poisson real MNAR data
saveRDS(pois_real_minMaxMiss_trim, file = "./data/missingDatasets/pois_real_MinMaxMiss_trim.rds")
## save poisson real MAR data
saveRDS(pois_real_randMiss_trim, file = "./data/missingDatasets/pois_real_randMiss_trim.rds")
...
# elements that have the response variable ranging from the lowest amount of 
# missing data to the highest proportion of missing data. 
saveRDS(gauss_sim_randMiss_full_A, file = "./data/missingDatasets/gauss_sim_randMiss_full_A.rds")
saveRDS(gauss_sim_randMiss_full_B, file = "./data/missingDatasets/gauss_sim_randMiss_full_B.rds")
...
saveRDS(gauss_sim_minMaxMiss_full, file = "./data/missingDatasets/gauss_sim_minMaxMiss_full.rds")

## save gaussian real MNAR data
saveRDS(gauss_auSable_minMaxMiss_full, file = "./data/missingDatasets/gauss_real_auSable_MinMaxMiss_full.rds")
## save gaussian real MAR data
saveRDS(gauss_auSable_randMiss_full, file = "./data/missingDatasets/gauss_real_auSable_randMiss_full.rds")

## save gaussian real MNAR data
saveRDS(gauss_badger_minMaxMiss_full, file = "./data/missingDatasets/gauss_real_badger_MinMaxMiss_full.rds")
## save gaussian real MAR data
saveRDS(gauss_badger_randMiss_full, file = "./data/missingDatasets/gauss_real_badger_randMiss_full.rds")

## for trimmed data
saveRDS(gauss_sim_randMiss_trim_A, file = "./data/missingDatasets/gauss_sim_randMiss_trim_A.rds")
saveRDS(gauss_sim_randMiss_trim_B, file = "./data/missingDatasets/gauss_sim_randMiss_trim_B.rds")

## save gaussian simulated MNAR data
saveRDS(gauss_sim_minMaxMiss_trim, file = "./data/missingDatasets/gauss_sim_minMaxMiss_trim.rds")

## save gaussian real MNAR data
saveRDS(gauss_auSable_minMaxMiss_trim, file = "./data/missingDatasets/gauss_real_auSable_MinMaxMiss_trim.rds")
## save gaussian real MAR data
saveRDS(gauss_auSable_randMiss_trim, file = "./data/missingDatasets/gauss_real_auSable_randMiss_trim.rds")

## save gaussian real MNAR data
saveRDS(gauss_badger_minMaxMiss_trim, file = "./data/missingDatasets/gauss_real_badger_MinMaxMiss_trim.rds")
## save gaussian real MAR data
saveRDS(gauss_badger_randMiss_trim, file = "./data/missingDatasets/gauss_real_badger_randMiss_trim.rds")
